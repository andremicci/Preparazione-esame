...
Vector OdeSolver::m_eqDiff(unsigned int i, double t, vector<MatPoint> p,string s)
{
  ...
  return F;
    ...
    return pow(consts::c,2)*m_p[i].P()*(1/m_p[i].E());
  }
...
void OdeSolver::Solve()
{
  ...
  {  
    ...
    for (unsigned int i = 0; i < m_p.size(); i++)
    {
      k1[i] = m_h * m_eqDiff(i,m_t,m_p,"r");
      w1[i] = m_h * m_eqDiff(i, m_t, m_p,"p");
    }
    ...
    for (unsigned int i = 0; i < m_p.size(); i++)
    {
      p2[i].P(m_p[i].P() + w1[i] * (0.5));
      p2[i].R(m_p[i].R() + k1[i] * (0.5));
    }
    for (unsigned int i = 0; i < m_p.size(); i++)
    {
      k2[i] = m_h *m_eqDiff(i,m_t,p2,"r");

      w2[i] = m_h * m_eqDiff(i, m_t, p2,"p");
    }
    for (unsigned int i = 0; i < m_p.size(); i++)
    {
